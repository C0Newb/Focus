...
class ConfigurationManager {
	 ...
	 * @type {Map<string, (ConfigItem|FocusConfig|User|Shoot)>}
	 */
	#cachedItems = new Map();
	...
	constructor(configDirectory, encryptionKey) {
		...
		this.#log = Focus.logMan.getLogger("ConfigMan");
	 ...
	 * @param {(ConfigItem|FocusConfig|User|Shoot)} configClass Which class to initialize. This is the class type that will be returned
	 * @return {(ConfigItem|FocusConfig|User|Shoot)}
	 */
	loadConfig(configName, configClass, isPath) {
		...
		try {
			...
			if (isPath !== true) // Config name provided
				...
				if (configClass === User)
					filePath = path.join(global.userData, configName);
				else if (configClass === Shoot)
					filePath = path.join(global.shootsData, configName);
				else
	...
	rekey(newKey) {
		if (typeof newKey === "boolean") {
			newKey = (newKey === true)? NeptuneCrypto.randomString(Focus.config.encryption.newKeyLength, 33, 220) : "";
		}
		...
		return new Promise((resolve, reject) => {
			...
			function walkDir(dir) { 
				...
				if (rekeyed) {
					if (newKey === "" || newKey === undefined)
						keytar.deletePassword("Focus","ConfigKey").then(() => {
							ugh.#encryptionKey = newKey;
							Focus.config.encryption.enabled = false;
							Focus.config.save();
						...
						keytar.setPassword("Focus","ConfigKey",newKey);
						ugh.#encryptionKey = newKey;
						Focus.config.encryption.enabled = true;
						Focus.config.save();
			...
			try {
				if (fs.existsSync(this.#configDirectory + "/FocusConfig.json"))
					if (updateFile(this.#configDirectory + "/FocusConfig.json") === true) // Maybe risky ..
						filesUpdate.push(this.#configDirectory + "/FocusConfig.json");
			} catch (err) {
				revert();
				reject(err);
				return;
	...
	writeFileContentsSync(data, filePath) {
		...
		if (Focus.config.encryption.enabled && (this.#encryptionKey !== undefined && this.#encryptionKey !== ""))
